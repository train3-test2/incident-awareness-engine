...
def test_한글_사용자명이_독립_토큰일_때만_치환된다():
    ...
    record = {
        "EventData": {
            "Image": "C:\\Users\\김현희\\a.exe",
            "CommandLine": "abc김현희def",
        }
    }
    replacements = build_replacements("PC", "김현희")
    ...
    assert result["EventData"]["Image"] == "C:\\Users\\labuser\\a.exe"
    assert result["EventData"]["CommandLine"] == "abc김현희def"


def test_호스트명이_FQDN_형태에서도_치환된다():
     # given - Sysmon NetworkConnect 가 기록하는 <host>.localdomain 형태
    record = {
        "Computer": "DESKTOP-J0SU0L8",
        "EventData": {
            "SourceHostname": "DESKTOP-J0SU0L8.localdomain",
            "DestinationHostname": "DESKTOP-J0SU0L8.localdomain",
        },
    }
    replacements = build_replacements("DESKTOP-J0SU0L8", "khh09")
    ...
    assert "DESKTOP-J0SU0L8" not in sanitized
...
def test_중간_경로에_Users_가_있어도_치환된다():
    ...
    line = json.dumps({"EventData": {"Image": "D:\\Data\\Users\\khh09\\a.exe"}})
    replacements = build_replacements("PC", "khh09")
